...
// Drifts inward (the gravity motif) everywhere except the forces view, where it's a calm
// scattered field — there, distance-from-centre already encodes power, so an inward pull would conflict.
export function GlobalField({ view }: { view: View }) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  useGravityField(canvasRef, undefined, view === 'forces' ? 'scattered' : 'inward')
